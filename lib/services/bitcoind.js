...
Bitcoin.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
  ...
  function queryHeader(err, blockhash) {
    ...
    var errMsg;
    self._tryAllClients(function(client, done) {
      for(var i =0; i<_toFetch;i++){
        self.getBlockHeader(blockhash, function(err, res){
          if (err) {
            hasErr=true;
            errMsg= err;
            i=_toFetch;
            ...
            res
            && res.hasOwnProperty('nextHash')
            && res.nextHash != 'undefined'
            && res.nextHash!=blockhash
          ){
            headers.push(res);
            blockhash = res.nextHash;
          }else{
            i=_toFetch;//Used to quit the loop
          }
        });
      }
      if(hasErr){
        return done(self._wrapRPCError(errMsg));
      }else{
