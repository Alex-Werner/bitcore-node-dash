 ...
 * @param {Function} callback
 */
Bitcoin.prototype.getBlockHeaders = function(blockArg, callback) {
  var self = this;
  var _toFetch = 2000;
  ...
  function queryHeader(err, blockhash) {
    ...
    self._tryAll(function(done) {
      self.client.getBlockHeaders(blockhash, _toFetch, function(err, response) {
        if (err) {
          return done(self._wrapRPCError(err));
        }
        var result = response.result;
        ...
        for (var i = 0; i < result.length; i++) {
          headers.push({
            ...
            chainWork: result[i].chainwork,
            prevHash: result[i].previousblockhash,
            nextHash: result[i].nextblockhash,
            merkleRoot: result[i].merkleroot,
            time: result[i].time,
            medianTime: result[i].mediantime,
